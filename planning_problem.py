    ...
    def get_successors(self, state):
        ...
        for action in self.actions:
            ...
            if action.all_preconds_in_list(state):
                new_state = set(state)
                for prop in action.get_delete():
                    new_state.remove(prop)
...
def max_level(state, planning_problem):
    ...
    while not planning_problem.is_goal_state(graph[level].get_proposition_layer().get_propositions()):
        ...
        next_level.expand_without_mutex(graph[level])
...
def level_sum(state, planning_problem):
    ...
    while not planning_problem.is_goal_state(graph[level].get_proposition_layer().get_propositions()):
        ...
        graph.append(next_level)
        level += 1
        for prop in next_level.get_proposition_layer().get_propositions():
            if prop in planning_problem.goal:
                sub_goals += level
